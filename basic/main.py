...
# 设置默认编码为 UTF-8（解决 Windows 系统 GBK 编码问题）
...
# 设置 langsmith 环境变量（如果.env 文件中存在这些变量）
...
# OPENAI_CHAT_API_KEY = ""
# OPENAI_CHAT_MODEL = ""
# oneapi 相关配置 (通义千问为例) 根据自己的实际情况进行调整
...
logger.info(f"OPENAI_BASE_URL: {os.getenv('OPENAI_BASE_URL', '未设置')}")
logger.info(f"OPENAI_API_KEY: {'已设置' if os.getenv('OPENAI_API_KEY') else '未设置'}")
logger.info(f"AI_MODEL: {os.getenv('AI_MODEL', '未设置')}")
logger.info(f"LANGCHAIN_TRACING_V2: {os.getenv('LANGCHAIN_TRACING_V2', '未设置')}")
logger.info(f"LANGCHAIN_API_KEY: {'已设置' if os.getenv('LANGCHAIN_API_KEY') else '未设置'}")
logger.info(f"===================")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        with open(PROMPT_TEMPLATE_TXT_SYS, 'r', encoding='utf-8') as f:
            prompt_template_system = PromptTemplate(template=f.read())
        with open(PROMPT_TEMPLATE_TXT_USER, 'r', encoding='utf-8') as f:
            prompt_template_user = PromptTemplate(template=f.read())
...
async def chat_completions(request: ChatCompletionRequest):
    ...
    try:
        ...
        # 检查 result.content 是否为 None
        if result.content is None:
            logger.warning("模型返回的内容为空，使用空字符串代替")
            raw_response = ""
        else:
            raw_response = result.content
                
        formatted_response = str(format_response(raw_response))
